...
LOCALEDOMAIN = 'mobile'
MOBILEAPPDIR = os.path.join(settings.PROJECT_DIR, "..", "mobileapp", "www")
...
class BuildFile(object):
    ...
    """
    re_gettext = [re.compile(r"(gettext\((['\"]).+?\2\))"),
                re.compile(r"(ngettext\((['\"]).+?\2,\s*(['\"]).+?\3,\s*\d+\))"),]
    ...
    def preprocess(self):
        ...
        if self.path[-10:] == ".tmpl.html":
            content = prepare_js_for_gettext(self.preprocess_tmpl(src_data))
        else:
    ...
    def preprocess_tmpl(self, src):
        out = []
        for line in src.split("\n"):
            gtxt = []
            for r in self.re_gettext:
                for found in r.findall(line):
                    gtxt.append(found[0])

            if gtxt:
                out.append('; '.join(gtxt)+';')
            else:
                out.append("\n")
...
"--locale, --exclude or --all options.")
    ...
    def handle(self, *args, **options):
        ...
        exts = extensions if extensions else ['html', 'js']
    ...
    def build_potfiles(self):
        ...
        """
        file_list = self.find_files(MOBILEAPPDIR)
        self.remove_potfiles()
    ...
    def copy_plural_forms(self, msgs, locale):
        ...
        for domain in domains:
            ...
            if os.path.exists(django_po):
                ...
                if m:
                    plural_form_line = force_text(m.group('value'))
                    if self.verbosity > 1:
