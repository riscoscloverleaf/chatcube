...
class IKTemplateBackend(object):
    """
    mostly copy/paste of original
    only uuid saved in IKMailUUID header
    ...
    def _render_email(self, template_name, context,
        ...
        render_context['settings'] = settings
    ...
    def get_email_message(self, template_name, context, from_email=None, to=None,
                          ...
                          template_prefix=None, template_dir=None,
                          attachments=None):
...
templated_email_backend = IKTemplateBackend()


def can_send_email_to(member):
    return not member.settings.get_value("unsubscribed") and not member.settings.get_value("email_error")


def send_email_template(template_name, context, email, mail_type='generic', render_only=False, from_email=None, member=None):
    if not from_email:
        from_email = settings.DEFAULT_FROM_EMAIL

    if render_only:
        return templated_email_backend.get_email_message(template_name, context, from_email=from_email,
                                                        to=[email])
    else:

        headers = {'IKMailType': mail_type,
                   "List-ID": "list.ds."+settings.EMAIL_DOMAIN,
                   "Precedence": "bulk"
                   }
        if member:
            headers["List-Unsubscribe"] = mark_safe(
                build_absolute_uri(
                    reverse("ik:unsubscribe", kwargs={"id": member.get_member_hash()})))
        return templated_email_backend.send(template_name, from_email, [email], context,
                                            headers=headers)


def send_email_text(recipients, subject, message):
