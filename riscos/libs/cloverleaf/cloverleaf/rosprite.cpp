...
void rosprite_destroy_sprite(struct rosprite* sprite) {
...
void rosprite_destroy_sprite_area(struct rosprite_area* sprite_area)
{
	uint32_t i;
	for (i = 0; i < sprite_area->sprite_count; i++) {
        rosprite_destroy_sprite(sprite_area->sprites[i]);
	}
...
static rosprite_error rosprite_get_mode(uint32_t sprite_mode_word, struct rosprite_mode* result)
{
	...
	if (spriteType != 0) {
        ...
        case 10:
            mode.color_model = ROSPRITE_RGB565;
            mode.colorbpp = 16;
            break;
		default:
	...
	for (y = 0; y < sprite->height; y++) {
		...
		for (x = 0; x < row_max_bit; x += bpp) {
			...
			if (old_has_alpha != has_alpha_pixel_data && (y > 0 || x_pixels > 0)) {
			    //Logger::debug("old_has_alpha != has_alpha_pixel_data %d %d %d", y, x, x_pixels);
	...
	case 16:
        {
            ...
            if (mode->color_model == ROSPRITE_RGB565) {
                /* incoming format is b_0000000000000000bbbbbggggggrrrrr */
                red   = pixel & 63;
                green = (pixel & (31 << 6)) >> 6;
                blue  = (pixel & (31 << 11)) >> 11;
...
roscreen rosprite_get_screen_info() {
    ...
    screen.width = vals[4] + 1;
    screen.height = vals[5] + 1;
    screen.bpl = ((screen.width << vals[1]) + 7) >> 3;
    screen.stride = vals[3];
    screen.xeig = vals[7];
    screen.yeig = vals[8];
    ...
    screen.bits_per_pixel = 1 << vals[1];
    screen.depth = 1 << vals[1];
    screen.true_colour_flag = (vals[1] >= 4);
...
int rosprite_capture_screenshoot(struct rosprite **sprite, int x1, int y1, int x2, int y2) {
    int result;
    osspriteop_area *sprbuffer;
    osspriteop_header *sprheader;
    roscreen screen = rosprite_get_screen_info();
    if (x1 == 0 && x1 == 0 & x2 == 0 && y2 == 0) {
        x2 = screen.width;
        y2 = screen.height;
    }
    int width = x2 - x1;
...
//    Logger::debug("capture %dx%d - %dx%d (%dx%d)\n",  x1, y1, x2, y2, width, height);

    int total_size = sizeof(osspriteop_area) + sizeof(osspriteop_header) +
                     (width * height * 4) + 2048 + 256;
    sprbuffer = (osspriteop_area *)malloc(total_size);
    sprbuffer->used = 16;
    sprbuffer->first = 16;
    sprbuffer->sprite_count = 0;
    sprbuffer->size = 0;
    osspriteop_get_sprite_user_coords(osspriteop_NAME, sprbuffer, "scrshoot", screen.true_colour_flag ? 0:1, x1 * 2, y1 * 2, x2 * 2 - 1, y2 * 2 - 1);
    sprheader = (osspriteop_header *)((char*)sprbuffer + 16);
//    Logger::debug("Sprite w=%d h=%d left_off=%d right_off=%d mode=%x\n",
//                 sprheader->width, sprheader->height,
//                 sprheader->left_bit, sprheader->right_bit,
//                 sprheader->mode);
//    osspriteop_save_sprite_file(512, sprbuffer, "<ChatCube$Dir>.cache.screenspr");
    struct rosprite_mem_context *rosprctx;
    struct rosprite_area *sprite_area;
    rosprite_create_mem_context((uint8_t *)sprbuffer + sizeof(int), total_size - sizeof(int), &rosprctx);
    result = rosprite_load(rosprite_mem_reader, rosprctx, &sprite_area);
    if (result == ROSPRITE_OK) {
        *sprite = sprite_area->sprites[0];
        free(sprite_area->sprites);
        if (sprite_area->extension_size > 0) free(sprite_area->extension_words);
        free(sprite_area);
    } else {
    ...
    rosprite_destroy_mem_context(rosprctx);
    free(sprbuffer);
    return result;
}